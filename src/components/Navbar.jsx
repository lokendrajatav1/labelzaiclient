    ...
    >
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center gap-10">
          <Link to="/" className="text-[15px] font-semibold text-slate-900 hover:text-brand-600 transition-colors">Home</Link>
          <a href="/#about" className="text-[15px] font-semibold text-slate-900 hover:text-brand-600 transition-colors">About Us</a>
          ...
          >
            <button className="flex items-center gap-1 text-[15px] font-semibold text-slate-900 hover:text-brand-600 transition-colors py-2">
              Services <ChevronDown size={14} className={`transition-transform duration-300 ${activeDropdown === 'services' ? 'rotate-180' : ''}`} />
            </button>
            <div className={`absolute top-full -left-20 w-[600px] bg-white border border-slate-100 shadow-2xl rounded-2xl p-6 grid grid-cols-2 gap-4 transition-all duration-300 origin-top ${activeDropdown === 'services' ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-2 invisible'}`}>
          ...
          >
            ...
            <div className={`absolute top-full -left-20 w-[600px] bg-white border border-slate-100 shadow-2xl rounded-2xl p-6 grid grid-cols-2 gap-4 transition-all duration-300 origin-top ${activeDropdown === 'industries' ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-2 invisible'}`}>
          ...
          <Link to="/career" className="text-[15px] font-semibold text-slate-900 hover:text-brand-600 transition-colors">Careers</Link>
          <Link to="/contact" className="text-[15px] font-semibold text-slate-900 hover:text-brand-600 transition-colors">Contact</Link>
        </div>

        {/* Actions */}
        <div className="hidden lg:flex items-center gap-5">
          <Link to="/contact" className="text-[15px] font-semibold bg-brand-600 text-white px-8 py-2.5 rounded-full shadow-lg shadow-brand-600/20 hover:bg-brand-700 hover:shadow-brand-600/40 transition-all active:scale-95">
            Get Started
          </Link>
        </div>
        ...
        {/* Mobile Menu Toggle */}
        <button 
          className="lg:hidden relative z-50 text-slate-900 p-2"
      ...
      <div className={`lg:hidden fixed inset-0 bg-white z-40 transition-transform duration-500 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="pt-24 px-6 h-full flex flex-col overflow-y-auto pb-10">
          <div className="flex flex-col gap-8">
            <Link to="/" className="text-2xl font-semibold text-slate-900" onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
            <a href="/#about" className="text-2xl font-semibold text-slate-900" onClick={() => setIsMobileMenuOpen(false)}>About Us</a>
            
            <div className="space-y-6">
              <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">Our Services</h3>
              <div className="grid grid-cols-1 gap-5">
                {services.map((item, idx) => (
                  <Link key={idx} to={item.link || "/#services"} className="flex items-center gap-4 text-lg font-semibold text-slate-700 hover:text-brand-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <div className="space-y-6">
              <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">What we Serve</h3>
              <div className="grid grid-cols-1 gap-5">
                {industries.map((item, idx) => (
                  <Link key={idx} to={item.link} className="flex items-center gap-4 text-lg font-semibold text-slate-700 hover:text-brand-600 transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
                    <span className="text-brand-600">{item.icon}</span>
                    {item.title}
                  </Link>
                ))}
            ...
            <Link to="/career" className="text-2xl font-semibold text-slate-900" onClick={() => setIsMobileMenuOpen(false)}>Careers</Link>
            <Link to="/contact" className="text-2xl font-semibold text-slate-900" onClick={() => setIsMobileMenuOpen(false)}>Contact</Link>
          </div>
          
          <div className="mt-auto pt-10">
            <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block w-full py-4 text-center rounded-2xl bg-brand-600 text-white font-semibold shadow-xl shadow-brand-600/30">Get Started</Link>
          </div>
        </div>
      </div>
    </nav>
